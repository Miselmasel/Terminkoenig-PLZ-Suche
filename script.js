...
// Werden komplett aus der Wertung genommen: nicht gezeichnet, keine Auswahl
// per Umkreissuche, keine Orte in Listen/Exporten, keine Entfernungen.
// Inseln MIT Straßenanbindung (Fehmarn, Nordstrand, Rügen, Usedom) bleiben.
// Bounding-Boxen: [latMin, latMax, lonMin, lonMax]
var INSEL_BOXEN = [
...
function exportCSV() {
  ...
  var header = '﻿PLZ-Bereich;PLZ;Ort;Bundesland;Einwohner;Betriebe_ca;Preisklasse' + (eigenePlzRef ? ';Entfernung_' + eigenePlzRef.plz + '_km' : '');
  var lines = [header];
  keys.forEach(function(prefix) {
    ...
    if (plzDB) {
      ...
      if (matches.length === 0) {
        lines.push(prefix+'xx;;;;'+betr+';'+pkLabel+(eigenePlzRef ? ';' : ''));
      } else {
        matches.forEach(function(e){
          ...
          lines.push([prefix+'xx',e.plz,e.ort,e.bundesland,ew,betr,pkLabel].join(';') + distStr);
        });
      }
    } else {
...
fetch(GEO_URL)
  ...
  .then(function (data) {
    ...
    // dadurch verschwinden sie optisch und aus Auswahl/Umkreissuche
    ...
    if (window._urlPresetGebiete) {
      applyUrlPresetGebiete(window._urlPresetGebiete);
      window._urlPresetGebiete = null;
    }
    updateSidebar();
    addLabels();
  })
...
function applyUrlPreset(data) {
  var typVal = data.typ || 'interessent';
  var typRadio = document.querySelector('input[name="fmTyp"][value="' + typVal + '"]');
  if (typRadio) { typRadio.checked = true; onFmTypChange(); }
  if (data.vorname) { var el = document.getElementById('fmVorname'); if (el) el.value = data.vorname; }
  if (data.nachname) { var el = document.getElementById('fmNachname'); if (el) el.value = data.nachname; }
  if (data.eigenePlz) {
    var ep = document.getElementById('fmEigenePlz');
    if (ep) { ep.value = data.eigenePlz; updateEigenePlzMarker(); }
  }
  if (typVal === 'kunde') {
    if (data.kundennummer) { var el = document.getElementById('fmKundennummer'); if (el) el.value = data.kundennummer; }
    if (data.vertragsnummer) { var el = document.getElementById('fmVertragsnummer'); if (el) el.value = data.vertragsnummer; }
  }
}

function applyUrlPresetGebiete(gebiete) {
  if (!Array.isArray(gebiete) || !gebiete.length || !geoL) return;
  gebiete.forEach(function(plz3) {
    if (!sel[plz3]) {
      sel[plz3] = true;
      geoL.eachLayer(function(layer) {
        if (layer.feature && layer.feature.properties && layer.feature.properties.plz === plz3) {
          layer.setStyle({ fillColor: SEL_COLOR, fillOpacity: 0.7 });
        }
      });
    }
  });
...
(function() {
  ...
  fetch('link.php?get=' + encodeURIComponent(token))
    ...
    .catch(function(e) { console.warn('Shortlink-Fehler:', e); });
})();
...
// ===== Overlay: Landkreise =====
var LK_GEO_URL = "https://raw.githubusercontent.com/isellsoap/deutschlandGeoJSON/main/4_kreise/1_sehr_hoch.geo.json";
...
map.getPane("lkPane").style.pointerEvents = "none"; // PLZ3-Polygone darunter bleiben immer klickbar
...
function buildEmailCSVTable(eigenePlzRef) {
  ...
  keys.forEach(function(prefix) {
    ...
    if (plzDB) {
      var matches = plzDB.filter(function(e){return e.plz.substring(0,3)===prefix;});
      if (matches.length === 0) {
        var row = [prefix+'xx','','','','',betr,pkCell];
        if (eigenePlzRef) row.push('');
        rows.push(row);
      } else {
        matches.forEach(function(e){
          var ew = Math.round(parseInt(e.einwohner||0) / 1000) * 1000;
          var row = [prefix+'xx', e.plz, e.ort, e.bundesland, ew.toLocaleString('de-DE'), betr, pkCell];
  ...
  h += '<tr style="background:#642d7b;color:#fff;"><th>PLZ-Bereich</th><th>PLZ</th><th>Ort</th><th>Bundesland</th><th>Einwohner ca.</th><th>Betriebe ca.</th><th>Preisklasse</th>';
    ...
    if (plzDB) {
      ...
      if (matches.length === 0) {
        ...
        matches.forEach(function(e) {
          var ew = Math.round(parseInt(e.einwohner||0) / 1000) * 1000;
          var distStr = eigenePlzRef ? (';' + Math.round(haversine(eigenePlzRef.lat, eigenePlzRef.lon, e.lat, e.lon))) : '';
          lines.push(prefix+'xx;'+e.plz+';'+e.ort+';'+e.bundesland+';'+ew+';'+betr+';'+pkLabel+distStr);
        });
      }
    } else {
